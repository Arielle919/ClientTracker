...
class TestEnteringClients < ClientTest
  def test_valid_client_information_gets_printed
    command = "./clienttracker add 'Sam Adams' --appointment 01/20/2014 --task 'Sign Docs' --need_appointment no task_completed yes"
    expected = "A client named Sam Adams, 01/20/2014, Sign Docs, no, yes, was created."
    assert_command_output expected, command
  end

  def test_valid_client_gets_saved
    execute_popen("./clienttracker add 'Sam Adams' --appointment 01/20/2014 --task 'Sign Docs' --need_appointment no task_completed yes --environment test")
    client = Client.all
    expected = ["Sam Adams", "01/20/2014", "Sign Docs", "no", "yes"]
  ...
  def test_error_message_for_missing_appointment
    command = "./clienttracker 'Sam Adams' --task 'Sign Docs' --need_appointment no task_completed yes"
